...
import pcd.ass02part01.simtrafficexamples.version1.Flag;
import pcd.ass02part01.simtrafficexamples.version1.StartSynch;
...
public abstract class AbstractSimulation {
	...
	public void run() {
		try {
			...
			int poolSize = (Math.min(agents.size(), nThread)) + 1;
			...
			while (nSteps < numSteps) {
				...
				try {
					for(List<AbstractAgent> part : parts) {
						//executor.execute(new ComputeCarStepTask(part, dt));
						Future<?> future = executor.submit(new ComputeCarStepTask(part, dt));
						futures.add(future);
						i++;
						//System.out.println("Step " + nSteps + " Task " + i);
					}
				} catch (Exception e) {
					e.printStackTrace();
				}
